...
do
        ...
        current_value=`echo "${key_value_input}" | cut -d "," -f 4`
                search_key=`echo $search_key | sed -e 's/(/\\(/g' -e 's/)/\\)/g'`
        search_key=">$search_key<"
                echo modified ${search_key1}
        key_value=`echo "${key_value_input}" | cut -d "," -f 5`
        echo "-- -- Start Tag = '$start_tag' and End Tag = '$value_tag'"
        echo "-- -- Search key = '$search_key', old value = $current_value and new Value = '$key_value'"
        ...
        do
                ...
                export start_tag
                export value_tag
                search_key1=`echo $search_key | sed -e 's/(/\\(/g' -e 's/)/\\)/g'`
                #echo modified ${search_key1}
                export search_key
			...
			print FILENAME >> "result";
				...
				print "flag = ",flag >> "result" ;
                        ...
                        if ( flag == 2 && $0 ~ ENVIRON["value_tag"] && $0 ~ ENVIRON["current_value"] ){
                                OFS=""
